...
func get(){
	...
	resp,err :=http.Get("https://jsonplaceholder.typicode.com/todos/12")
	if err !=nil{
		log.Fatalln(err)
	}  /*if err != nil {
	    fmt.Println(err)
...
func  post(){
	...
	todo :=Todo{1,20,"lorem ipsum dolor sit amet",true}
...
func put() {
	...
	todo := Todo{1, 6, "have you look on it", true}
	jsonReq, err := json.Marshal(todo)
...
func delete() {
	...
	todo := Todo{12, 2, "lorem ipsum dolor sit amet", true}
	jsonReq, err := json.Marshal(todo)
	req, err := http.NewRequest(http.MethodDelete, "https://jsonplaceholder.typicode.com/todos/12", bytes.NewBuffer(jsonReq))
	client := &http.Client{}
	resp, err := client.Do(req)
	if err != nil {
		log.Fatalln(err)
